...
def solve(y, A_fun, AT_fun, lambda_l1, reshape_img_fun, base_folder, show_img_progress=False, alpha=0.2, max_iter=100, solver_tol=1e-6, ori_img=None):
    ...
    def compute_p_inv_A(b, z0):
        (z,info) = sp.sparse.linalg.cgs(A_cgs, vec(b), x0=vec(z0), tol=1e-3, maxiter=100) # inv(Q) b
        if info > 0:
    ...
    def compute_init(b, z0):
        (z,info) = sp.sparse.linalg.cgs(A_cgs_init, vec(b), x0=vec(z0), tol=1e-2) # inv(A_T_A) b
        if info > 0:
            print('cgs convergence to tolerance not achieved')
        elif info <0:
            print('cgs gets illegal input or breakdown')
    ...
    for iter in range(max_iter):
        ...
        q = soft_threshold(Wzu, lambda_l1/alpha) # softthres(W (z+u))
        ...
        if ori_img is None:
            ...
            print('iter = %d: obj_ls = %.3e  |x-z| = %.3e  u_norm = %.3e  psnr = %.3e' % (iter, obj_ls, x_z, u_norm, psnr))
