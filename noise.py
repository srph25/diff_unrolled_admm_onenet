...
def add_noise(x, local_random,
    ...
    def actually_add_noise(x):
        ...
        if continuous_noise > 0:
            ...
            noise = local_random.randn(x_shape[0], x_shape[1], x_shape[2]) * std # random gaussian, zero mean, std stddev, so when multiplied, we scale the stddev

        if use_spatially_varying_uniform_on_top > 0: # multiply gaussian with uniform noise map
            for channel in range(x_shape[2]):
                ...
                lowres_noise_map = local_random.rand(low_res_row,low_res_col) * 2 * uniform_max - uniform_max # generate uniform noise [-uniform_max, uniform_max]
                highres_noise_map = sp.misc.imresize(lowres_noise_map, [x_shape[0], x_shape[1]], interp='bicubic', mode='F') # upsample uniform noise
                noise[:,:,channel] *= highres_noise_map # multiply noise with upsampled uniform noise map, i.e., scale the stddev
    ...
    def create_blocky_image(x, min_resize_ratio):
        ratio = local_random.rand() * (0.95 - min_resize_ratio) + min_resize_ratio # random scale [0.2, 0.95]
        tmp = sp.misc.imresize(x, ratio, interp='nearest') # downsample with random scale
        y = sp.misc.imresize(tmp, [x.shape[0], x.shape[1]], interp='nearest').astype(float) / 255.0 # upsample to original size
    ...
    r = local_random.rand()
    if r < prob_block:
        y = create_blocky_image(x, min_resize_ratio=0.2) # random downsample and upsample

    result = actually_add_noise(y) # add spatially varying stddev gaussian noise
